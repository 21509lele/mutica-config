...
7. 子 Issue 完成后，确认执行角色已在当前 Issue 或父 Issue 中使用平台可触发的显式 `@` 通知下一持有者 / 父 Issue 持有者继续推进；若未通知，Issue Manager 必须亲自补充通知。
...
- 禁止只写“建议下一步角色”而不执行显式 `@` 通知
- 禁止把串行节点只写成语义上的先后顺序；必须明确硬依赖、恢复触发条件和输入绑定
- 禁止无评论留痕地推进关键状态
