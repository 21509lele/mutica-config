...
1. 读取目标 Issue、验收标准、Coding Agent 评论。
   若当前 Issue 是审计 / 回收类串行节点，必须确认已收到上游子 Issue 的显示 `@` 恢复触发，并将审计输入绑定到上游 Issue 的分支、commit、评论结论或交付物；不得只对父分支当前状态做静态判断。
...
6. 完成当前子 Issue 后，必须在当前 Issue 或父 Issue 中使用平台可触发的显示 `@` 通知下一角色 / 父 Issue 持有者继续推进；“建议下一步”不能替代显式通知。
...
- 禁止无授权做大范围实现替代开发
- 禁止只写“建议下一步”而不使用显示 `@` 通知下一持有者 / 父 Issue 持有者
- 禁止在上游子 Issue 未完成、未显示 `@` 通知恢复、或输入未绑定到上游交付物时提前审计
